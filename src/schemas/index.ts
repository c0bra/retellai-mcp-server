...
export const CreatePhoneCallInputSchema = z.object({
	...
	overrideAgentId: z
		.string()
		.optional()
		.describe("For this particular call, override the agent used with this agent id"),
	overrideAgentVersion: z
		.number()
		.optional()
		.describe("For this particular call, override the agent version used with this version"),
...
export const ListCallsInputSchema = z.object({
	...
	startTimestamp: z.number().optional().describe("Filter calls after this timestamp"),
	endTimestamp: z.number().optional().describe("Filter calls before this timestamp"),
...
export const AddKnowledgeBaseSourceInputSchema = z.object({
	...
	sourceConfig: z.record(z.string(), z.unknown()).describe("Configuration for the source"),
});
...
export const CreateRetellLLMInputSchema = z.object({
	...
	model: z
		...
		.optional()
		.describe("Select the underlying text LLM. If not set, would default to gpt-4o"),
	s2s_model: z
		.enum(["gpt-4o-realtime", "gpt-4o-mini-realtime"])
		.optional()
		.describe("Select the underlying speech to speech model. Can only set this or model, not both"),
	model_temperature: z
		.number()
		.optional()
		.describe("If set, will control the randomness of the response. Value ranging from [0,1]"),
	model_high_priority: z
		.boolean()
		.optional()
		.describe("If set to true, will use high priority pool with more dedicated resource"),
	...
	begin_message: z.string().optional().describe("First utterance said by the agent in the call"),
	default_dynamic_variables: z
		.record(z.string(), z.string())
		.optional()
		.describe("Default dynamic variables represented as key-value pairs of strings"),
