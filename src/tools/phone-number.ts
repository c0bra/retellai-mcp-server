...
export const registerPhoneNumberTools = (server: McpServer, retellClient: Retell) => {
	server.registerTool(
		"list_phone_numbers",
	...
	server.registerTool(
		...
		createToolHandler(async (data: z.infer<typeof CreatePhoneNumberInputSchema>) => {
			...
			const phoneNumber = await retellClient.phoneNumber.create(createPhoneNumberDto);
	...
	server.registerTool(
		...
		createToolHandler(async (data: z.infer<typeof GetPhoneNumberInputSchema>) => {
			const phoneNumber = await retellClient.phoneNumber.retrieve(data.phoneNumber);
			return transformPhoneNumberOutput(phoneNumber);
		})
	);
	...
	server.registerTool(
		...
		createToolHandler(async (data: z.infer<typeof UpdatePhoneNumberInputSchema>) => {
